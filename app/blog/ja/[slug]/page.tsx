...
export async function generateStaticParams() {
  const dir = path.join(process.cwd(), "content/blog/ja");
...
export default async function BlogPostJP({
  params,
}: {
  ...
  const fullPath = path.join(process.cwd(), "content/blog/ja", `${slug}.md`);
        ...
        <div className="ef-blog-inner">
          <div className="ef-blog-topbar">
            ...
            <div className="ef-blog-toggle">
              <Link href={`/blog/${slug}`} className="ef-blog-toggle-link">
                English
              </Link>

              <span className="ef-blog-toggle-current">日本語</span>
            </div>
          </div>

          <div className="ef-blog-kicker">English Fitness Blog</div>

          <h1 className="ef-blog-title">{data.title}</h1>

          {data.date ? <p className="ef-blog-date">{data.date}</p> : null}
